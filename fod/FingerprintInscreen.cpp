...
Return<int32_t> FingerprintInscreen::getDimAmount(int32_t suggest) {
    ...
    if (suggest > 0) {
        if (suggest < 127) {
            ...
            dimAmount = get(DIM_AMOUNT_PATH, suggest) + 53;
            if (dimAmount >= 237) {
                dimAmount = 0;
            }
        }
...
Return<int32_t> FingerprintInscreen::getDimAmountPressed(int32_t suggest) {
    LOG(INFO) << "dimAmount3 = " << suggest;
    if (suggest > 0) {
        if (suggest < 127) {
            dimAmount = 255 - suggest - 16;
        } else {
            dimAmount = 88 + get(DIM_AMOUNT_PATH, suggest);
            if (dimAmount > 237) {
                dimAmount = 255;
            }
        }
